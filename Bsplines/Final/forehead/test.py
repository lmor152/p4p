...
xgridFile = 'test4VM/xgrid_test4.csv'
ygridFile = 'test4VM/ygrid_test4.csv'
PhiFile = 'test4VM/Phi.csv'
...
if definegrid:
    ...
    plt.axvline(max(tx) + 1, color = 'k')
    plt.axhline(min(ty), color = 'k')
    plt.axhline(max(ty) + 1, color = 'k')

    xcoords = [min(tx), max(tx) + 1] 
    ycoords = [min(ty), max(ty) + 1]
