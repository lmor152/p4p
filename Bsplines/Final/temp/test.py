...
invp = np.c_[X.flatten(), Y.flatten(), Z.flatten()]
inversep = pca.inverse_transform(invp)
itx = inversep[:,0].reshape(np.shape(X))
ity = inversep[:,1].reshape(np.shape(Y))
itz = inversep[:,2].reshape(np.shape(Z))
